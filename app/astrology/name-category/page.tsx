...
import { NAME_CATEGORY_DATA } from '@/constants/menuData';
import { ArrowLeft, BookOpen } from 'lucide-react';
import Link from 'next/link';

export default function NameCategoryPage() {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className={`text-3xl md:text-4xl font-bold mb-4 bg-gradient-to-r from-indigo-900 to-purple-800 bg-clip-text text-transparent ${currentLanguage === 'ne' ? 'font-nepali' : ''}`}>
            {currentLanguage === 'ne' ? 'नामको पहिलो अक्षरबाट वर्ग हेर्ने तालिका' : 'Name First Letter Category Table'}
          </h1>
          <p className={`text-lg text-gray-600 max-w-2xl mx-auto ${currentLanguage === 'ne' ? 'font-nepali' : ''}`}>
            {currentLanguage === 'ne' 
              ? 'नामको पहिलो अक्षर अनुसार वर्ग र शत्रु पहिचान गर्नुहोस्'
              : 'Identify category and enemies based on the first letter of your name'
            }
          </p>
        </div>
        ...
        <Card className="shadow-xl border-0 overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-green-500 to-emerald-500 text-white">
            <CardTitle className={`text-2xl text-center ${currentLanguage === 'ne' ? 'font-nepali' : ''}`}>
              {currentLanguage === 'ne' ? 'अक्षर र वर्ग तालिका' : 'Letter and Category Table'}
            </CardTitle>
          </CardHeader>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className={`px-6 py-4 text-left text-lg font-semibold text-gray-900 border-b ${currentLanguage === 'ne' ? 'font-nepali' : ''}`}>
                      {currentLanguage === 'ne' ? 'अक्षर' : 'Letters'}
                    </th>
                    <th className={`px-6 py-4 text-left text-lg font-semibold text-gray-900 border-b ${currentLanguage === 'ne' ? 'font-nepali' : ''}`}>
                      {currentLanguage === 'ne' ? 'वर्ग (शत्रु)' : 'Category (Enemy)'}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {NAME_CATEGORY_DATA.map((item, index) => (
        ...
        <Card className="mt-8 bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
          <CardContent className="p-6">
            ...
            <div className={`space-y-3 text-blue-800 ${currentLanguage === 'ne' ? 'font-nepali' : ''}`}>
              ...
              <p>
                {currentLanguage === 'ne' 
                  ? '३. तपाईंको वर्ग र शत्रु जान्नुहोस्'
                  : '3. Know your category and enemy'
                }
              </p>
              <p>
                {currentLanguage === 'ne' 
                  ? '४. यो जानकारी व्यापार र सम्बन्धमा उपयोगी हुन्छ'
                  : '4. This information is useful in business and relationships'
                }
              </p>
            </div>
          </CardContent>
          ...
          <Card className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white border-0">
            <CardContent className="p-8">
              <h3 className={`text-2xl font-bold mb-4 ${currentLanguage === 'ne' ? 'font-nepali' : ''}`}>
                {currentLanguage === 'ne' ? 'थप विस्तृत जानकारीको लागि' : 'For More Detailed Information'}
              </h3>
              <p className={`text-indigo-100 mb-6 ${currentLanguage === 'ne' ? 'font-nepali' : ''}`}>
                {currentLanguage === 'ne' 
                  ? 'व्यक्तिगत नाम विश्लेषण र सुझावको लागि परामर्श लिनुहोस्'
                  : 'Get consultation for personal name analysis and suggestions'
                }
              </p>
              <Link href="/booking">
